...
def navigation(
    ...
    for item in NAV_ITEMS:
        ...
        if item["key"] in {"infoproducts", "crm", "creator_day", "central"} and not workspace_has_infoproducts_access(workspace, user):
            continue
...
def finance_snapshot(workspace: Workspace, month_filter: str | None = None) -> dict:
    ...
    return {
        ...
        "finance_desktop": {
            ...
            "fixed_cost_paid": currency(fixed_cost_paid),
            "fixed_cost_remaining": currency(fixed_cost_remaining),
            "fixed_cost_items_count": len(fixed_costs),
